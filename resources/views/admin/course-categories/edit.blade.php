...
@section('page-title', 'Chỉnh sửa Danh mục Khóa học')
    ...
    <div class="card">
        <div class="card-header">
            <h5 class="card-title mb-0">
                <i class="fas fa-edit me-2"></i>Chỉnh sửa danh mục: <span class="text-primary">{{ $courseCategory->name }}</span>
            </h5>
        </div>
        <div class="card-body">
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                           id="name" name="name" value="{{ old('name', $courseCategory->name) }}" 
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror" 
                                              ...
                                              placeholder="Nhập mô tả ngắn về danh mục">{{ old('description', $courseCategory->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" class="form-control @error('icon') is-invalid @enderror" 
                                                       id="icon" name="icon" value="{{ old('icon', $courseCategory->icon) }}" 
                                                       placeholder="fas fa-code">
                                                @error('icon')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                            <div class="form-text">Sử dụng Font Awesome icons</div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="number" class="form-control @error('order') is-invalid @enderror" 
                                                   id="order" name="order" value="{{ old('order', $courseCategory->order) }}" 
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group color-picker-group">
                                                <input type="color" class="form-control form-control-color @error('color') is-invalid @enderror" 
                                                       id="color" name="color" value="{{ old('color', $courseCategory->color) }}" 
                                                       title="Chọn màu sắc cho danh mục">
                                                <input type="text" class="form-control color-hex-input @error('color') is-invalid @enderror" 
                                                       id="color_hex" name="color_hex" value="{{ old('color', $courseCategory->color) }}" 
                                                       pattern="^#[0-9A-Fa-f]{6}$" 
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="info-item mb-2">
                                        ...
                                        <div>
                                            <code class="text-primary">{{ $courseCategory->slug }}</code>
                                        </div>
                                    </div>
                                    <div class="info-item mb-2">
                                        <small class="text-muted">Tổng khóa học:</small>
                                        <div>
                                            <span class="badge bg-info">{{ $courseCategory->courses_count }} khóa học</span>
                                        </div>
                                    </div>
                                    <div class="info-item mb-3">
                                        <small class="text-muted">Ngày tạo:</small>
                                        <div>
                                            <small class="text-muted">{{ $courseCategory->created_at->format('d/m/Y H:i') }}</small>
                                        </div>
                                    </div>
                                    <div class="info-item">
                                        <small class="text-muted">Cập nhật cuối:</small>
                                        <div>
                                            <small class="text-muted">{{ $courseCategory->updated_at->format('d/m/Y H:i') }}</small>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="preview-section">
                                    ...
                                    <div class="preview-category p-3 rounded border">
                                        <div class="d-flex align-items-center mb-2">
                                            @if($courseCategory->icon || old('icon'))
                                            <div class="me-2 preview-icon" style="color: {{ old('color', $courseCategory->color) }};">
                                                <i class="{{ old('icon', $courseCategory->icon) }} fa-lg"></i>
                                            </div>
                                            @endif
                                            <span class="badge me-2 preview-badge" 
                                                  style="background: {{ old('color', $courseCategory->color) }}; color: white;">
                                                <span class="preview-name">{{ old('name', $courseCategory->name) ?: $courseCategory->name }}</span>
                                            </span>
                                            <small class="text-muted preview-count">{{ $courseCategory->courses_count }} khóa học</small>
                                        </div>
                                        <p class="preview-description mb-0 small text-muted">
                                            {{ old('description', $courseCategory->description) ?: ($courseCategory->description ?: 'Mô tả danh mục sẽ hiển thị ở đây...') }}
                                        </p>
                                    </div>
                                ...
                                <div class="mt-4 p-3 bg-light rounded border">
                                    ...
                                    <p class="small mb-2">Bạn có thể xóa danh mục này vì nó không chứa khóa học nào.</p>
                                    <form action="{{ route('admin.course-categories.destroy', $courseCategory) }}" 
                                          method="POST" 
                                          onsubmit="return confirm('Bạn có chắc chắn muốn xóa danh mục {{ $courseCategory->name }}?')">
                                        @csrf
                                        @method('DELETE')
                                ...
                                <div class="mt-4 p-3 bg-warning bg-opacity-10 rounded border border-warning">
                                    ...
                                    <p class="small mb-0">Không thể xóa danh mục này vì đang chứa {{ $courseCategory->courses_count }} khóa học.</p>
                                </div>
                                @endif
                            </div>
                        </div>
                ...
                <div class="row mt-4">
                    <div class="col-12">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div>
                                <a href="{{ route('admin.course-categories.create') }}" class="btn btn-outline-success me-2">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePreview() {
        ...
        const name = nameInput.value || '{{ $courseCategory->name }}';
        const description = descriptionInput.value || '{{ $courseCategory->description }}' || 'Mô tả danh mục sẽ hiển thị ở đây...';
        const icon = iconInput.value || '{{ $courseCategory->icon }}';
