...
@section('content')
    @php
        ...
        $requiresPaidCheckout = $displayPayableAmount > 0;
        $walletBalance = 0;
        ...
        <div class="row g-4">
                        ...
                        <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap mb-3">
                            ...
                            <div class="text-end">
                                @if($displayDiscountAmount > 0)
                                    ...
                                    <div class="small text-success fw-semibold">Tiết kiệm {{ number_format($displayDiscountAmount) }} VND</div>
                                @else
            ...
            <div class="col-lg-4">
                <div class="card shadow-sm border-0 sticky-top course-show-sidebar" id="intakes">
                    <div class="card-body">
                        <h3 class="text-primary fw-bold mb-3">{{ number_format($displayPayableAmount) }} VND</h3>

                        @if($displayDiscountAmount > 0)
                            <div class="small text-muted text-decoration-line-through mb-1">{{ number_format($displayBaseAmount) }} VND</div>
                            <div class="small text-success fw-semibold mb-3">Tiết kiệm {{ number_format($displayDiscountAmount) }} VND</div>
                        @endif
                        ...
                        @if(! empty($promotionPreview['automatic_options']))
                            <div class="mb-3">
                                <div class="small text-muted mb-2">Ưu đãi tự động khả dụng</div>
                                    ...
                                    @foreach($promotionPreview['automatic_options'] as $offer)
                                        <div class="border rounded p-2 bg-light-subtle">
                                            <div class="d-flex justify-content-between align-items-start gap-2">
                                                ...
                                                <span class="badge bg-success-subtle text-success border">-{{ number_format($offer['discount_amount']) }}đ</span>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        @auth
                            @if(! $isEnrolled)
                                <div class="mb-3">
                                    <label for="discount-code-input" class="form-label fw-semibold">Mã giảm giá</label>
                                    <input type="text" id="discount-code-input" class="form-control" value="{{ $selectedDiscountCode }}" placeholder="Ví dụ: CHAOMUNG10" data-discount-code-source>
                                    <small class="text-muted d-block mt-2">Nhập mã rồi bấm đăng ký để hệ thống áp dụng khi tạo đơn.</small>
